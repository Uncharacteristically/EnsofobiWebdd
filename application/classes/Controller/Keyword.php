...
class Controller_Keyword extends Controller_Template {
    ...
    function action_select2(){
        $q=$this->request->query("q");
        $resultsg=DB::select("keyword","id")->from("keywords")->where("status","=","1")->and_where("keyword","LIKE","%$q%")->order_by("keyword")->execute();
        foreach($resultsg as $r){
            $keywords[]=array("text"=>$r["keyword"],"id"=>$r["id"]);
        }
        $this->template->content=json_encode($keywords); 
    }
