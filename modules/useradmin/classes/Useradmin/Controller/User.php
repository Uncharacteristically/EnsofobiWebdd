...
class Useradmin_Controller_User extends Controller_App {
	...
	public function action_profile()
	{
		...
		//$view->set('user', Auth::instance()->get_user());
                ...
                $profile=DB::select("users.id","username","email","profiles.*")->from("users")->join("profiles")->on("users.id","=","user_id")->where("user_id","=",$user->id)->execute()->as_array();
                $fields=array();
               ...
               $this->template->bind("fields",$fields);
              // $this->template->bind("myProfile",$profile["0"]);
	}
	...
	{
        $view = View::factory('content/register');
        $profileType=$this->request->param("id");
		$view->set("profileType",$profileType);
		...
		$this->template->mainContent = $view;
	}
	...
	public function action_unregister()
	{
		...
		if (is_numeric($id) && isset($_POST['confirmation']) && $_POST['confirmation'] == 'Y')
		{
			...
			$this->redirect(Session::instance()->get_once('returnUrl','user/profile'));
		}
	...
	public function action_login()
	{
		...
		else
		{
			...
			$view = View::factory('user/newlogin');
			// If there is a post and $_POST is not empty
